...
DESTINATIONS: dict[str, tuple[str, int, int | None]] = {
    ...
    "store": ("the Kwik-Snak convenience store", 5, None),
...
class Taxi(Room):
    ...
    def _destination(self, p: Parsed) -> str | None:
        for name in DESTINATIONS:
            if p.has(name) and not p.has("look"):
                return name
        return None
    ...
    def said(self, game: Game, p: Parsed) -> bool:
        ...
        elif p.words[0] in ("enter", "open", "stand", "get", "drop") and (len(p.words) == 1 or p.has("out", "door")):
    ...
    def _leave(self, game: Game, fare: int) -> None:
        if fare:
            ...
            game.print("You slide out onto the sidewalk outside Rooster's. The cab idles, hopeful.")
            game.new_room(10)
